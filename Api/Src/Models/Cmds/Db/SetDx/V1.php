...
namespace MTM\RedisApi\Models\Cmds\Db\SetDx;
...
class V1 extends Base
{
	...
	public function getRawCmd()
	{
		...
		$strCmd	.= " if redis.call('exists','".$this->getKey()."') == 0 then return 'SETDX-FAIL'";
		$strCmd	.= " elseif redis.call('type','".$this->getKey()."').ok ~= 'string' then return 'SETDX-INVALID'";
		$strCmd	.= " else redis.call('set', '".$this->getKey()."', '".$data."') return 'SETDX-OK'";
		$strCmd	.= " end";
	...
	public function parse($rData)
	{
		...
		if ($rData == "SETDX-OK") {
			...
			$this->setResponse(false)->setException(new \Exception("Key is not type string: ".$this->getKey()));
		} else {
